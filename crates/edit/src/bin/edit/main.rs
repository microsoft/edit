...
fn handle_args(state: &mut State) -> apperr::Result<bool> {
    let scratch = scratch_arena(None);
    let mut paths = BorrowedVec::empty();
...
fn write_terminal_title<'a>(arena: &'a Arena, output: &mut BorrowedString<'a>, state: &mut State) {
    ...
    ctx.modal_begin("warning", loc(LocId::WarningDialogTitle));
    {
        ...
        if over_limit {
            ...
            let label2 = {
                ...
                let mut label = BorrowedString::empty();
