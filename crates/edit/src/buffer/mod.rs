...
impl TextBuffer {
    ...
    fn find_parse_replacement<'a>(
        ...
        replacement: &[u8],
    ) -> BorrowedVec<'a, RegexReplacement<'a>> {
        ...
        let mut text = BorrowedVec::empty();
        let mut text_beg = 0;
    ...
    pub fn render(
        ...
        for y in 0..height {
            let scratch = scratch_arena(None);
            let mut line = BorrowedString::empty();
    ...
    fn write(&mut self, text: &[u8], at: Cursor, raw: bool) {
        ...
        let mut offset = 0;
        let scratch = scratch_arena(None);
        let mut newline_buffer = BorrowedString::empty();
