...
pub fn score_fuzzy<'a>(
...
) -> (i32, BorrowedVec<'a, usize>) {
    if haystack.is_empty() || needle.is_empty() {
        // return early if target or query are empty
        return (NO_MATCH, BorrowedVec::empty());
    }
    ...
    let mut scores = BorrowedVec::empty();
    let mut matches = BorrowedVec::empty();
    ...
    let mut positions = BorrowedVec::empty();
...
fn map_chars<'a>(arena: &'a Arena, s: &str) -> BorrowedVec<'a, char> {
    let mut chars = BorrowedVec::empty();
