...
pub fn get_available_encodings() -> &'static Encodings {
    ...
    unsafe {
        if ENCODINGS.all.is_empty() {
            ...
            let mut preferred = BorrowedVec::empty();
            let mut alternative = BorrowedVec::empty();
...
impl<'pivot> Converter<'pivot> {
    ...
    fn append_nul<'a>(arena: &'a Arena, input: &str) -> BorrowedString<'a> {
...
impl Regex {
    ...
    pub unsafe fn new(pattern: &str, flags: i32, text: &Text) -> Result<Self> {
        ...
        unsafe {
            let scratch = scratch_arena(None);
            let mut utf16 = BorrowedVec::empty();
            let mut status = icu_ffi::U_ZERO_ERROR;
...
pub fn fold_case<'a>(arena: &'a Arena, input: &str) -> BorrowedString<'a> {
    // OnceCell for people that want to put it into a static.
    #[allow(static_mut_refs)]
    let casemap = unsafe {
    ...
    if !casemap.is_null() {
        let f = assume_loaded();
        let mut status = icu_ffi::U_ZERO_ERROR;
        let mut output = BorrowedVec::empty();
        let mut output_len;
        ...
        if status.is_success() && output_len > 0 {
            ...
            return unsafe { BorrowedString::from_utf8_unchecked(output) };
        }
    }

    let mut result = BorrowedString::from_str(arena, input);
