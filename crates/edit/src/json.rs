...
impl<'a, 'i> Parser<'a, 'i> {
    ...
    #[cold]
    fn parse_escape(&mut self, result: &mut BorrowedString<'a>) -> Result<(), ParseError> {
        if self.pos >= self.bytes.len() {
    ...
    #[cold]
    fn parse_unicode_escape(&mut self, result: &mut BorrowedString<'a>) -> Result<(), ParseError> {
    ...
    fn parse_array(&mut self, depth: usize) -> Result<Value<'a>, ParseError> {
        let mut values = BorrowedVec::empty();
    ...
    fn parse_object(&mut self, depth: usize) -> Result<Value<'a>, ParseError> {
        let mut entries = BorrowedVec::empty();
        let mut expects_comma = false;
