...
unsafe fn lines_bwd_raw(
    ...
    return unsafe {
        let func = LINES_BWD_DISPATCH.load(std::sync::atomic::Ordering::Relaxed);
...
#[cfg(any(target_arch = "x86_64", target_arch = "loongarch64"))]
static LINES_BWD_DISPATCH: std::sync::atomic::AtomicPtr<()> =
    std::sync::atomic::AtomicPtr::new(lines_bwd_dispatch as *mut _);

#[cfg(target_arch = "x86_64")]
fn lines_bwd_dispatch(
    beg: *const u8,
    end: *const u8,
    line: CoordType,
    line_stop: CoordType,
    ...
    LINES_BWD_DISPATCH.store(func as *mut _, std::sync::atomic::Ordering::Relaxed);
