...
pub fn read_stdin(arena: &Arena, mut timeout: time::Duration) -> Option<BorrowedString<'_>> {
    ...
    let mut text = BorrowedString::empty();
...
pub fn preferred_languages<'a>(arena: &'a Arena) -> BorrowedVec<'a, &'a str> {
    ...
    let langs = BorrowedString::from_utf16_lossy(arena, langs).leak();
