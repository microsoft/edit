...
impl Tui {
    ...
    pub fn render<'a>(&mut self, arena: &'a Arena) -> BorrowedString<'a> {
    ...
    fn render_node(&mut self, node: &mut Node) {
        ...
        if node.attributes.bordered {
            // ┌────┐
            {
                ...
                let mut fill = BorrowedString::empty();
                fill.push(&*scratch, '┌');
                fill.push_repeat(
                    &*scratch,
                    '─',
    ...
    fn render_styled_text(
        ...
        if overflow == Overflow::Clip || target_width >= actual_width {
            ...
            let mut modified = BorrowedString::empty();
    ...
    pub fn debug_layout<'a>(&mut self, arena: &'a Arena) -> BorrowedString<'a> {
        let mut result = BorrowedString::empty();
...
impl<'a> Context<'a, '_> {
    ...
    pub fn styled_label_begin(&mut self, classname: &'static str) {
        ...
        self.tree.last_node.borrow_mut().content = NodeContent::Text(TextContent {
            text: BorrowedString::empty(),
            chunks: BorrowedVec::empty(),
    ...
    fn menubar_shortcut(&mut self, shortcut: InputKey) {
        ...
        if shortcut_letter.is_ascii_uppercase() {
            let mut shortcut_text = BorrowedString::empty();
...
struct TableContent<'a> {
    columns: BorrowedVec<'a, CoordType>,
    cell_gap: Size,
}
...
struct TextContent<'a> {
    text: BorrowedString<'a>,
    chunks: BorrowedVec<'a, StyledTextChunk>,
    overflow: Overflow,
}
