...
//! ## Output formats
//!
//! - `generate_assembly()` - human-readable assembly listing with optional VT100 colors
//! - `generate_rust()` - Rust source file with embedded bytecode for `include!`
//!
//! ## Rust codegen details
//!
//! The generated file contains:
//! - `enum HighlightKind` with `TryFrom<u32>` for safe conversion from VM output
//! - `LANGUAGES: &[Language]` - entrypoints for each language definition
//! - `FILE_ASSOCIATIONS: &[(&str, &Language)]` - path patterns to languages
//! - `ASSEMBLY: [u8; N]` - raw bytecode
//! - `CHARSETS: [[u16; 16]; M]` - charset lookup tables (transposed for SIMD)
//! - `STRINGS: [&str; K]` - string table for prefix matching
//!
//! The charset transposition (`[bool; 256]` → `[u16; 16]`) happens here, not in the engine.
//! This means the engine reads the SIMD-friendly format directly.
//!
//! ## Mermaid diagram
//!
//! The generated Rust file includes a Mermaid flowchart in a comment, generated by
//! `Compiler::as_mermaid()`. Useful for debugging the IR graph structure. It's huge
//! for complex languages though.
//!
//! ## TODO
//!
//! - Could generate C/C++ output for non-Rust consumers.
//! - The label lookup (`HashMap<usize, &str>`) is rebuilt for every output format. Should
//!   be computed once during assembly.
