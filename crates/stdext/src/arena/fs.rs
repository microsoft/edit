...
use super::Arena;
use crate::collections::{BorrowedString, BorrowedVec};

pub fn read_to_vec<P: AsRef<Path>>(arena: &'_ Arena, path: P) -> io::Result<BorrowedVec<'_, u8>> {
    fn inner<'a>(arena: &'a Arena, path: &Path) -> io::Result<BorrowedVec<'a, u8>> {
        let mut file = File::open(path)?;
        let mut vec = BorrowedVec::empty();
...
pub fn read_to_string<P: AsRef<Path>>(arena: &Arena, path: P) -> io::Result<BorrowedString<'_>> {
    fn inner<'a>(arena: &'a Arena, path: &Path) -> io::Result<BorrowedString<'a>> {
        let vec = read_to_vec(arena, path)?;
        BorrowedString::from_utf8(vec).map_err(|_| {
