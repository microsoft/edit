...
unsafe impl Allocator for Arena {
    fn realloc(
        ...
        if old_size != 0 && unsafe { old_ptr.add(old_size) == self.base.add(self.offset.get()) } {
            if new_size > old_size {
                self.alloc_raw(new_size - old_size, 1);
            } else {
                self.offset.set(self.offset.get() - old_size + new_size);
            }
            return NonNull::slice_from_raw_parts(old_ptr, new_size);
        }

        // Otherwise, we have to allocate a new area and copy it over.
        if new_size > old_size {
            unsafe {
                let ret = self.alloc_raw(new_size, align);
                ptr::copy_nonoverlapping(old_ptr.as_ptr(), ret.as_ptr() as *mut _, old_size);
                ret
            }
        } else {
