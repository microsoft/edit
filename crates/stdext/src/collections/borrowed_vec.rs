...
impl<'a, T> BorrowedVec<'a, T> {
    ...
    fn grow(&mut self, alloc: &'a dyn Allocator, cap: usize, add: usize) {
        ...
        self.cap = new_ptr.len() / std::mem::size_of::<T>();
    }
...
impl<T> Default for BorrowedVec<'_, T> {
    fn default() -> Self {
        Self::empty()
    }
}

impl<T> Deref for BorrowedVec<'_, T> {
    type Target = [T];
...
impl<T> DerefMut for BorrowedVec<'_, T> {
    #[inline]
...
impl<T> PartialEq<BorrowedVec<'_, T>> for BorrowedVec<'_, T>
where
    T: PartialEq,
{
    #[inline]
    fn eq(&self, other: &BorrowedVec<T>) -> bool {
        self.deref() == other.deref()
    }
}

impl<T> Eq for BorrowedVec<'_, T> where T: PartialEq {}

impl<T> PartialEq<[T]> for BorrowedVec<'_, T>
where
    T: PartialEq,
{
    #[inline]
...
impl<T> fmt::Debug for BorrowedVec<'_, T>
where
    T: fmt::Debug,
{
...
impl<'a, T> IntoIterator for BorrowedVec<'a, T> {
    type Item = T;
...
impl<'a, T> IntoIterator for &'a BorrowedVec<'a, T> {
    type Item = &'a T;
...
impl<'a, T> IntoIterator for &'a mut BorrowedVec<'a, T> {
    type Item = &'a mut T;
...
/// Owning iterator over the elements of a [`BorrowedVec`].
