...
use edit::arena::{ArenaString, scratch_arena};
...
macro_rules! gen_l10n {
    (
        $(
            $lang_name_str:literal:
            $lang_name_1:ident $(: [$($prefix:literal),+])?
        ),+;
        $(
            $(#[doc = $string_id_doc:literal])?
            $string_name:ident: {
                $($lang_name_2:ident: $string:expr),+$(,)?
            }
        ),+$(,)?
    ) => {
        const STRING_COUNT: usize = count_sizes!($($string_name)+);
        const LANG_COUNT: usize = count_sizes!($($lang_name_1)+);
        ...
        impl LangId {
            ...
            pub const fn map() -> &'static [(&'static str, Self)] {
                use LangId::*;
                &[
                    $(
                        (stringify!($lang_name_1), $lang_name_1),
                    )+
                ]
            }
            pub const fn prefix_map() -> &'static [(&'static str, Self)] {
                use LangId::*;
                &[
                    $(
                        (stringify!($lang_name_1), $lang_name_1),
            ...
            pub const fn name(&self) -> &'static str {
                ...
                match self {
                    $(
                        $lang_name_1 => $lang_name_str,
                    )+
                }
            }
        }

        type LangLut = [[&'static str; LANG_COUNT]; STRING_COUNT];

        const S_LANG_LUT: LangLut = generate_lut();

        const fn generate_lut() -> LangLut {
            use LangId::*;
            use LocId::*;
            let mut lut: LangLut = [[""; LANG_COUNT]; STRING_COUNT];
            $(
                $(
                    lut[$string_name as usize][$lang_name_2 as usize] = $string;
                )+
            )+
            lut
        }
        ...
        #[test]
        fn ensure_all_strings_for_all_langs() {
            use LocId::*;
            $(
                assert_eq!(S_LANG_LUT.get($string_name as usize).is_some(), true);
                for (name, lang) in LangId::map() {
                    assert_eq!(S_LANG_LUT.get($string_name as usize).is_some(), true);
                    // `.get()` returns `Some("")`, but panics on `.unwrap()` because.
                    debug_assert_eq!(
                        S_LANG_LUT.get($string_name as usize).unwrap()
                            .get(*lang as usize).unwrap_or(&"").len() > 0,
                        true,
                        "{:?} is missing for language {}", $string_name, name
                    );
                }
            )+
        }
...
gen_l10n! {
    ...
    "Russian": ru,
    "Chinese (Simplfied)": zh_hans: ["zh_tw"],
    "Chinese (Traditional)": zh_hant: ["zh"];
    Ctrl: {
        en: "Ctrl",
        de: "Strg",
        es: "Ctrl",
    ...
    #[doc = "Shown when the clipboard size exceeds the limit for OSC 52"]
...
impl<'a> TryFrom<ArenaString<'a>> for LangId {
    type Error = ();

    fn try_from(value: ArenaString) -> Result<Self, Self::Error> {
        let value = value.replace("-", "_");
        for (prefix, lang) in LangId::prefix_map() {
            if value.starts_with_ignore_ascii_case(*prefix) {
                return Ok(*lang);
            }
        }
        Err(())
...
pub fn init() {
    ...
    let mut lang = LangId::first();
    for lang_str in lang_strs {
        if let Ok(new_lang) = LangId::try_from(lang_str) {
            lang = new_lang;
            break;
        }
    }
...
static mut S_LANG: LangId = LangId::first();
