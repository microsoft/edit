...
impl TextBuffer {
    ...
    pub fn move_selected_lines(&mut self, direction: MoveLineDirection) -> bool {
        ...
        // Insert the reordered text into the buffer
        let replacement_bounds = match direction {
            MoveLineDirection::Up => {
                TextBufferSelection { beg: target_beg.logical_pos, end: source_end.logical_pos }
            }
            MoveLineDirection::Down => {
                TextBufferSelection { beg: source_beg.logical_pos, end: target_end.logical_pos }
            }
        };
        self.edit_begin(
            HistoryType::Write,
            self.cursor_move_to_logical_internal(self.cursor, replacement_bounds.beg),
        );
        self.set_selection(Some(replacement_bounds));
...
mod tests {
    ...
    fn test_move_empty_line_up_from_bottom() {
        ...
        let last_line = test_lines.len() - 1;
        buffer.cursor_move_to_logical(Point { x: 0, y: last_line as i32 });
        ...
        assert_eq!(
            Point { x: 0, y: last_line as i32 - 1 },
            resulting_cursor,
            "Cursor position should be updated to the new line position"
        );
    }
    ...
    fn test_move_penultimate_line_down() {
        ...
        let penultimate_line = test_lines.len() - 2;
        buffer.cursor_move_to_logical(Point { x: 0, y: penultimate_line as i32 });
    ...
    fn test_move_selection_with_penultimate_line_down() {
        ...
        let penultimate_line = test_lines.len() as i32 - 2; // 0-indexed
    ...
    fn test_move_selection_with_ultimate_line_up() {
        ...
        let test_lines: Vec<&str> =
            vec!["Line 1", "Line 2", "Line 3", "Line 4", "Line 5", "Line 6"];
        let ultimate_line = test_lines.len() as i32 - 1; // 0-indexed
