...
impl Syscall for WindowsSys {
    ...
    fn read_stdin(arena: &Arena, mut timeout: time::Duration) -> Option<ArenaString<'_>> {
        ...
        loop {
            if timeout != time::Duration::MAX {
                ...
                match unsafe {
                    Threading::WaitForSingleObject(STATE.stdin, timeout.as_millis() as u32)
                } {
    ...
    fn open_stdin_if_redirected() -> Option<File> {
        unsafe {
            ...
            if !ptr::eq(STATE.stdin, handle) { Some(File::from_raw_handle(handle)) } else { None }
        }
    }
    ...
    fn file_id(file: Option<&File>, path: &Path) -> apperr::Result<FileId> {
        ...
        file_id_from_handle(file).or_else(|_| Ok(FileId::Path(fs::canonicalize(path)?)))
    }
    ...
    unsafe fn virtual_release(base: NonNull<u8>, _size: usize) {
        unsafe {
    ...
    fn load_icu() -> apperr::Result<LibIcu> {
        ...
        if const { const_ptr_u16_eq(LIBICUUC, LIBICUI18N) } {
            let icu = unsafe { Self::load_library(LIBICUUC)? };
            Ok(LibIcu { libicuuc: icu, libicui18n: icu })
        } else {
            let libicuuc = unsafe { Self::load_library(LIBICUUC)? };
            let libicui18n = unsafe { Self::load_library(LIBICUI18N)? };
